...
const BTN_LABELS: Record<string, string> = {
  ...
  completed: "Mark completed (applies stock)",
...
export function TransferStatusActions({ id, currentStatus }: Props) {
  const router = useRouter();
  const [loading, setLoading] = useState<string | null>(null);
  const [error, setError] = useState("");

  const allowed = TRANSFER_TRANSITIONS[currentStatus as TransferStatus] ?? [];
