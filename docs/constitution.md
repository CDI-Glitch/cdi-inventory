...
- **展示位置**：
  - Dashboard 一张统计卡「At-risk reservations」（有告警时变红），点「View all」进 Inventory Backorder。不在仪表板重复工作清单。
  - Inventory 页新增与 Forecast Mode 平行的独立模式（`?backorder=1`），只显示当前仓库里被标记的 SKU，多两列"Aged"/"Next supply"。两个模式互斥（不叠加显示），避免同一行同时塞入 Forecast 的多货柜列和 Aging 的年龄列导致过挤。仓库筛选在 Inventory 的 Location Tab。
