...
- [ ] Viewer/sales 直调 `GET /api/inventory/shortage-export`、`GET /api/products`、`GET /api/inventory/export` → shortage-export 应 403（内含到货 PO/ETA 数据，走 `canAccessIncoming`）；products 应返回时不含 `adminNotes`/Shopify 绑定字段；export 应 403（当前筛选库存快照，走 `canEditProduct`）
...
| 2026-09-15 | Inventory 需要按当前 Location Tab + category/search/status（或 Backorder 的 alert）导出该仓 SKU 快照给 editor/admin 用 Excel 打开，而不是全仓合并一张表 | `GET /api/inventory/export`（`canEditProduct`，复用 `buildInventoryView`）；Inventory 页「Export SKUs」紧挨 Adjust Stock |
