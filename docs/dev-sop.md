...
- [ ] Viewer/sales 直调 `GET /api/inventory/shortage-export`、`GET /api/products`、`GET /api/inventory/full-export` → shortage-export 应 403（内含到货 PO/ETA 数据，走 `canAccessIncoming`）；products 应返回时不含 `adminNotes`/Shopify 绑定字段；full-export 应 403（全量库存快照，走 `canEditProduct`）
...
| 2026-09-15 | Inventory 需要一份不受当前 Tab / 分页 / Backorder 筛选影响的全量 SKU 快照（On Hand 按仓拆列），给 editor/admin 用 Excel 打开 | `GET /api/inventory/full-export`（`canEditProduct`）；Inventory 页常驻「Export SKUs」按钮 |
